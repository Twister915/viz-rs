...
impl From<SampleRaw> for VizFloat {
    fn from(sample: SampleRaw) -> VizFloat {
        use SampleRaw::*;

        match sample {
            OneByte(b) => ((b as VizFloat) - 128.0) / 128.0,
...
impl ByteOrdering {
    ...
    where
        R: Read,
    {
        let bytes: [u8; 4] = self.read_n(reader, buf, 4)?.try_into()?;
        use ByteOrdering::*;
        Ok(match self {
    ...
    where
        R: Read,
    {
        let bytes: [u8; 2] = self.read_n(reader, buf, 2)?.try_into()?;
        use ByteOrdering::*;
        Ok(match self {
    ...
    fn i16_from<'a>(&self, buf: &'a [u8]) -> Result<(i16, &'a [u8])> {
        ...
        use ByteOrdering::*;
        let u16 = match self {
            LittleEndian => u16::from_le_bytes(data),
            BigEndian => u16::from_be_bytes(data),
        };
...
impl WavFile {
        ...
        P: AsRef<Path>,
    {
        ...
        if fmt_len < 16 {
            return Err(anyhow!("fmt chunk too short, got {} bytes", fmt_len));
        }
        ...
        if fmt_len > 16 {
            f.seek(SeekFrom::Current((fmt_len - 16) as i64))?;
        }
        ...
        if num_channels != 1 && num_channels != 2 {
            return Err(anyhow!(
                "bad number of channels (unsupported): {}",
                num_channels
            ));
        }
        ...
        let data_starts_at = f.stream_position()?;
...
impl Samples for WavFile {
    type Item = Channeled<SampleRaw>;

    fn seek_samples(&mut self, n: isize) -> Result<(), Error> {
        let new_sample_at = ((self.sample_at as isize) + n).clamp(0, self.num_samples as isize);
        let byte_offset =
            self.data_starts_at + ((new_sample_at as u64) * (self.block_align as u64));
        self.f.seek(SeekFrom::Start(byte_offset))?;
        self.sample_at = new_sample_at as usize;
        Ok(())
    }

    fn next_sample(&mut self) -> Result<Option<Self::Item>, Error> {
    ...
    R: Read + Seek,
{
    loop {
        ...
        let chunk_len = ordering.read_u32(reader, &mut buf[..])?;
        let seek_by = SeekFrom::Current((chunk_len + (chunk_len % 2)) as i64);
        reader.seek(seek_by)?;
    }
}
    ...
    R: Read,
{
    ...
    while !unseen_tag_bytes.is_empty() {
